...
/*
    사전 지식: OAuth
        - 제 3의 서비스에 계정 관리를 맡기는 방식. Naver/Kakao/Google로 로그인하기와 같은 방식
        ...
        관련 용어 정리
            ...
            인증 서버 (Authorization Server) - 클라이언트에게 리소스 오너의 정보에 접근할 수 있는
                토큰을 발급하는 역할을 하는 애플리케이션을 의미한다.
            클라이언트 애플리케이션 (Client Application) - 인증 서버에게 인증을 받고 리소스 오너의
                리소스를 사용하는 주체.

        OAuth를 사용하면 인증 서버에서 발급받은 토큰을 사용해서 리소스서버에 리소스오너의
        정보를 요청하고 응답받아 사용이 가능하다. 그렇다면 리소스오너의 정보를 어떻게 취득할 수 있는지 방법이 필요.

        리소스 오너 정보를 취득하는 네가지 방법
            1) 권한 부여 코드 승인 타입 (Authorizatoin Code Grant Type):
                OAuth 2.0에서 가장 대중적인 인증 방법. 클라이언트가 리소스에 접근하는 데 사용.
                권한에 접근할 수 있는 코드와 리소스 오너에 대한 액세스 토큰을 발급하는 방식.
            2) 암시적 승인 타입 (Implicit Grant Type)
                서버가 없는 자바스크립트 웹 애플리케이션 클라이언트에서 주로 사용.
                클라이언트가 요청을 보내면 리소스 오너의 인증 과정 이외에는 권한 코드 교환 등의
                별다른 인증 과정을 거치치 않고 액세스 토큰을 제공받는 방식.
            ...
            권한 부여 코드 승인 타입이란?
                애플리케이션 / 리소스 오너 / 리소스 서버 / 인증 서버가 어떤 순서로 인증을 하는지 알 필요가 있다.
     ...
     1. 권한 요청   |       5. 액세스 토큰으로 데이터에 접근| 인증 서버에
        ...
        1. 권한 요청 :
            ...
            즉 카카오나 구글 권한 서버에 요청을 보내는 것. 요청 URI는 권한 서버마다 다르지만,
            보통 클라이언트 ID, 리다이렉트 URI, 응답 타입 등을 파라미터로 보낸다.
            ex) 실제 요청에 쓰이는 요청 URI 예시.
            ...
            response_type - 클라이언트가 제공받길 원하는 응답 타입. 인증 코드를 받을 때는 code 값을 포함해야 한다.
            scope - 제공받고자 하는 리소스 오너의 정보 목록 (Google보다는 네이버나 카카오에 많이 보인다.)

        2. 데이터 접근용 권한 부여
            인증 서버에 요청을 처음 보내는 경우 사용자에게 보이는 페이지를 로그인 페이지로 변경하고,
            사용자의 데이터에 접근 동의를 얻는다. 이 과정은 최초 1회만 진행되고,
            이후에는 인증 서버에서 동의 내용을 저장하고 있기 떄문에 로그인만 진행한다.
            로그인에 성공하면 권한 부여 서버는 데이터에 접근할 수 있게 인증 및 권한 부여를 수신한다.

        3. 인증 코드 제공
            사용자가 로그인에 성공하면 권한 요청 시에 파라미터로 보낸 redirect_url로 리다이렉트된다.
        ...
        4. 액세스 코드 응답
            '인증 코드'를 받으면 '액세스 토큰'으로 교환한다.
            액세스 토큰은 로그인 세션에 대한 보안 자격을 증명하는 식별코드.
            보통 다음과 같이 토큰 POST 요청을 보낸다
        ...
        5. 액세스 토큰으로 API 응답 & 반환
            ...
            정보가 필요할 때마다 API 호출을 통해 정보를 가져오고 리소스 서버는 토큰이 유효한지 검사한 뒤에 응답합니다.
            ...
            대부분의 OAuth를 구현한 라이브러리는 이 흐름을 바탕으로 코드를 구현하기 때문에,
            흐름을 이해하고 넘어가는 것이 좋습니다.

        쿠키란?
            사용자가 어떤 웹사이트를 방문했을 때 해당 웹사이트의 서버에서
            여러분의 로컬 환경에 저장하는 작은 데이터를 의미함.
            이 값이 있기 때문에 이전에 방문한 적이 있는지 알 수 있고,
            이전에 로그인을 했다면 로그인 정보도 유지할 수 있는 것입니다.
        ...
        쿠키 관련 클래스 구현
            ...
            그때마다 쿠키를 생성하고 삭제하는 로직을 추가하면 불편하기 때문에,
            유틸리티로 사용할 쿠키 관리 클래스를 미리 구현한다.
