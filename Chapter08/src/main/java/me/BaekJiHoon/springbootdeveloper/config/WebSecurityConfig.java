...
/*
    1. 스프링 시큐리티의 모든 기능을 사용하지 않게 설정하는 코드
        ...
        h2 데이터를 확인하는데 사용하는 h2-console 하위 url을 상대로 .ignoring() 메서드

    2. 특정 HTTP 요청에 대한 웹 기반 보안을 구성. 이 메서드에서 인증 / 인가 및 로그인, 로그아웃
    ...
    4. 폼 기반 로그인을 설정
        loginPage() : 로그인 페이지 경로를 생성
        defaultSuccessUrl() : 로그인이 완료되었을 때 이동할 경로를 설정 -> /articles

    5. 로그아웃 설정.
        logoutSuccessUrl() : 로그아웃이 완료되었을 때 이동할 경로를 설정
        invalidHttpSession() : 로그아웃 이후에 세션을 전체 삭제할지 여부를 설정 -> 우리는 true로 설정.

    6. csrf 설정을 비활성화 - CSRF 공격을 방지하기 해서 사용하는 거넫, 실습을 편하게 하기 위하여 비활성화함.

    7. 인증 관리자 관련 설정. 사용자 정보를 가져올 서비스를 재정의하거나, 인증 방법 (LADP, JDBD 기반 인증 등)

    8. 사용자 서버를 설정
        userDetailService : 사용자 정보를 가져올 서비스를 설정. 이 때설정하는 서비스 클래스는 반드시
