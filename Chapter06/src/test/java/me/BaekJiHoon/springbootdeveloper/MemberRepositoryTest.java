...
import me.BaekJiHoon.springbootdeveloper.Member;
import me.BaekJiHoon.springbootdeveloper.MemberRepository;
...
class MemberRepositoryTest {
    ...
    /*
    ex) id가 2인 멤버의 이름을 BC로 바꾼다고 가정해보자.
    SQL문) UPDATE member SET name = 'BC' where id=2;
    JPA에서 데이터를 수정하는 방식은 추후 SQL에서 배우게 될 트랜잭션 내에서 데이터를 수행해야 한다.
    -> 메서드만 사용하는 것이 아니라 @Transactional Annotation을 추가해야한다.
    (SQL의 COMMIT, ROLLBACK(마지막 커밋 상태로 돌아가는 것),
    SAVEPOINT(특정 지점 지정 후 ROLLBACK TO ...를 통해 그 지점으로 돌아갈 수 있다.)와 연관되어 있다.)
     */
    @Sql("/insert-members.sql")
    @Test
    public void update(){
        Member member = memberRepository.findById(2L).get();
        member.changeName("BC"); //.get()을 해야 객체가 리턴된다.
        assertThat(memberRepository.findById(2L).get().getName()).isEqualTo("BC");
    }
    /*
        지금까지는 @Transactional Annotation이 존재하지 않는다
        -> @DataJPATest 애너테이션 때문. (여기에 @Transactional이 상속되어 있다.)
     */
}